...
def get_dashboard(
    ...
    """Get high-level metrics for the CA firm dashboard."""
    ...
    pending_docs = db.scalar(
        sa.select(sa.func.count(Document.id)).where(
            Document.firm_id == firm_uuid, Document.status.in_(["pending", "processing", "extracted"])
        )
    ) or 0
    extracted_docs = db.scalar(
        sa.select(sa.func.count(Document.id)).where(
            Document.firm_id == firm_uuid, Document.status == "ready_for_review"
        )
    ) or 0
    ...
    for event in recent_events:
        activity.append({
            "id": str(event.id),
            "message": f"Action {event.action} performed on {event.resource_type}",
    ...
    return {
        ...
        "processed_today": extracted_docs, # using extracted as pseudo-metric
        "documents_this_month": pending_docs + extracted_docs,
...
def list_clients(
    ...
    for client in clients:
        ...
        status_val = "pending" if doc_count == 0 else "ready"
        result.append({
            "id": str(client.id),
            "name": client.full_name,
            "pan_masked": f"xxxxxx{client.pan_last4}",
