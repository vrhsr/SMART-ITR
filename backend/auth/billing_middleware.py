...
class BillingMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if scope["method"] == "OPTIONS" or path.startswith("/api/billing/webhook") or path.startswith("/health"):
            await self.app(scope, receive, send)
            return

        auth_header = next((v for k, v in scope.get("headers", []) if k == b"authorization"), None)
