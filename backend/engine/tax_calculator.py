...
def calculate_new_regime_tax(gross_income_paise: int) -> dict[str, int]:
    """
    AY 2025-26 New Tax Regime (Section 115BAC — default from AY 2024-25).
    Standard deduction: ₹75,000 for salaried (Budget 2024).
    87A Rebate: Full rebate if POST-STANDARD-DEDUCTION taxable income <= ₹12,00,000.
    """
    # Standard deduction is always applied for salaried in new regime
    STD_DEDUCTION_RUPEES = 75_000
    gross_rupees = gross_income_paise // 100
    # 87A rebate applies on income AFTER standard deduction
    taxable_rupees = max(0, gross_rupees - STD_DEDUCTION_RUPEES)

    tax_rupees = 0.0
    # Slabs (AY 2025-26): 0-3L@0%, 3-7L@5%, 7-10L@10%, 10-12L@15%, 12-15L@20%, 15L+@30%
    ...
    # If taxable income (after standard deduction) <= ₹12,00,000 → full rebate (zero tax).
    # The cliff is sharp: ₹12,00,001 gives no rebate at all.
    if taxable_rupees <= 1_200_000:
        tax_before_cess = 0

    s_rate = _surcharge_rate(taxable_rupees, is_new_regime=True)
